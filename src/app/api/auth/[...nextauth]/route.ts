  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      credentials: {
        ...
        usarLdap: { label: "usarLdap", type: "text" },
      },
      async authorize(credentials) {
        ...
        console.log("usarLdap recibido:", usarLdap);

        const url = usarLdap
          ? "http://localhost:8080/ServidorApp-1.0-SNAPSHOT/api/usuarios/login"
          : "http://localhost:8080/ServidorApp-1.0-SNAPSHOT/api/usuarios/login-comun";
  ...
  callbacks: {
      ...
      if (account?.provider === "google") {
        try {
          const res = await fetch(
            `http://localhost:8080/ServidorApp-1.0-SNAPSHOT/api/usuarios/google-login`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
    ...
    async jwt({ token, user }) {
      if (user) {
        ...
        token.idPerfil = user.data?.idPerfil?.id;
        token.user = {
          id: user.data?.id,
          name: user.data?.nombre,
          email: user.data?.email,
          idPerfil: user.data?.idPerfil?.id,
