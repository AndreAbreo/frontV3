...
import {PaisModel, UbicacionModel} from '@/types';
...
const PaisDetail = () => {
    ...
    const { id } = useParams();
    const [pais, setPais] = useState<PaisModel | null>(null);

    useEffect(() => {
        if (id) {
            const fetchPais = async () => {
                const res = await fetch(`${API_BASE_URL}/paises/buscar?id=${id}`, {
                    headers: {
    ...
    if (!pais) return <div>...loading</div>;
